<script lang="ts">
  ...
  $: currentScale = ZOOM_SCALES.find(scale => scale.level === $zoomLevel) 
    ?? ZOOM_SCALES[4]; // Default to middle zoom if not found
  ...
  $: majorTicks = Array.from(
    { length: numberOfMajorTicks },
    (_, i) => {
